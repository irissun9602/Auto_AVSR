...
def get_device_index_from_uuid(uuid):
    ...
    for line in output:
        ...
        if gpu_uuid.strip() == uuid:
            return int(index.strip())
    raise ValueError(f"GPU with UUID {uuid} not found")

class InferencePipeline(torch.nn.Module):
    def __init__(self, cfg, detector="retinaface", device=None):
        super(InferencePipeline, self).__init__()
        self.modality = cfg.data.modality
        if device is None:
            gpu_uuid = 'GPU-e6a1af9f-c02c-18c0-db6f-af9ec6e25b34'  # 예제 UUID
            device_index = get_device_index_from_uuid(gpu_uuid)
            print(gpu_uuid)
