...
python multi_demo_test_mig.py data.modality='video' \
  ...
  file_path="filelist/filelist021.txt" \
  csv_file="test021.csv" \
  mig_uuid="${mig_uuids[21]}" &

python multi_demo_test_mig.py data.modality='video' \
  pretrained_model_path="${pretrained_model_path}" \
  file_path="filelist/filelist022.txt" \
  csv_file="test022.csv" \
  mig_uuid="${mig_uuids[22]}" &

python multi_demo_test_mig.py data.modality='video' \
  pretrained_model_path="${pretrained_model_path}" \
  file_path="filelist/filelist023.txt" \
  csv_file="test023.csv" \
  mig_uuid="${mig_uuids[23]}" &

python multi_demo_test_mig.py data.modality='video' \
  pretrained_model_path="${pretrained_model_path}" \
  file_path="filelist/filelist024.txt" \
  csv_file="test024.csv" \
  mig_uuid="${mig_uuids[24]}" &
python multi_demo_test_mig.py data.modality='video' \
  pretrained_model_path="${pretrained_model_path}" \
  file_path="filelist/filelist025.txt" \
  csv_file="test025.csv" \
  mig_uuid="${mig_uuids[25]}" &

python multi_demo_test_mig.py data.modality='video' \
  pretrained_model_path="${pretrained_model_path}" \
  file_path="filelist/filelist026.txt" \
  csv_file="test026.csv" \
  mig_uuid="${mig_uuids[26]}" &

python multi_demo_test_mig.py data.modality='video' \
  pretrained_model_path="${pretrained_model_path}" \
  file_path="filelist/filelist027.txt" \
  csv_file="test027.csv" \
  mig_uuid="${mig_uuids[27]}" &
