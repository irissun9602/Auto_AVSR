...
python multi_demo_test_mig.py data.modality='video' \
  ...
  file_path="filelist/filelist007.txt" \
  csv_file="test007.csv" \
  mig_uuid="${mig_uuids[7]}" &

python multi_demo_test_mig.py data.modality='video' \
  pretrained_model_path="${pretrained_model_path}" \
  file_path="filelist/filelist008.txt" \
  csv_file="test008.csv" \
  mig_uuid="${mig_uuids[8]}" &

python multi_demo_test_mig.py data.modality='video' \
  pretrained_model_path="${pretrained_model_path}" \
  file_path="filelist/filelist009.txt" \
  csv_file="test009.csv" \
  mig_uuid="${mig_uuids[9]}" &

python multi_demo_test_mig.py data.modality='video' \
  pretrained_model_path="${pretrained_model_path}" \
  file_path="filelist/filelist010.txt" \
  csv_file="test010.csv" \
  mig_uuid="${mig_uuids[10]}" &
python multi_demo_test_mig.py data.modality='video' \
  pretrained_model_path="${pretrained_model_path}" \
  file_path="filelist/filelist011.txt" \
  csv_file="test011.csv" \
  mig_uuid="${mig_uuids[11]}" &

python multi_demo_test_mig.py data.modality='video' \
  pretrained_model_path="${pretrained_model_path}" \
  file_path="filelist/filelist012.txt" \
  csv_file="test012.csv" \
  mig_uuid="${mig_uuids[12]}" &

python multi_demo_test_mig.py data.modality='video' \
  pretrained_model_path="${pretrained_model_path}" \
  file_path="filelist/filelist013.txt" \
  csv_file="test013.csv" \
  mig_uuid="${mig_uuids[13]}" &
