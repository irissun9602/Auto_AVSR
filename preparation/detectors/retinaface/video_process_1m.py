...
def linear_interpolate(landmarks, start_idx, stop_idx):
    start_landmarks = cp.array(landmarks[start_idx])
    stop_landmarks = cp.array(landmarks[stop_idx])
    delta = stop_landmarks - start_landmarks
    for idx in range(1, stop_idx - start_idx):
        landmarks[start_idx + idx] = cp.asnumpy(
...
def cut_patch(img, landmarks, height, width, threshold=5):
    center_x, center_y = cp.asnumpy(cp.mean(cp.array(landmarks), axis=0))
...
class VideoProcess:
    ...
    def crop_patch(self, video, landmarks):
        ...
        for frame_idx, frame in enumerate(video):
            ...
            smoothed_landmarks = cp.asnumpy(cp.mean(
                cp.array(
                    [
                        landmarks[x]
                        for x in range(
            ...
            smoothed_landmarks += cp.asnumpy(cp.mean(cp.array(landmarks[frame_idx]), axis=0)) - cp.asnumpy(cp.mean(cp.array(smoothed_landmarks), axis=0))
    ...
    def apply_affine_transform(
        ...
        transformed_landmarks = (
            cp.matmul(cp.array(landmarks), cp.array(transform[:, :2].transpose()))
            + cp.array(transform[:, 2].transpose())
        )
        return transformed_frame, cp.asnumpy(transformed_landmarks)
