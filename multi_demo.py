...
def main(cfg):
    ...
    with open(csv_file, mode='w', newline='') as file:
        writer = csv.DictWriter(file, fieldnames=['file_path', 'audio', 'video', 'gt'])
        ...
        with tqdm(total=len(file_paths)) as pbar:
            for file_path in file_paths:
                try:
                    ...
                    # Generate path to corresponding JSON file
                    json_file_path = file_path.replace('real.mp4', 'real.json')
                    json_file_path = json_file_path.replace('train', 'train_metadata')

                    # Read the JSON file and extract transcripts
                    gt_transcript = ""
                    if os.path.exists(json_file_path):
                        with open(json_file_path, 'r') as json_file:
                            json_data = json.load(json_file)
                            gt_transcript = " ".join([word_info["word"] for word_info in json_data["transcripts"]])
                    ...
                    if existing_entry:
                        ...
                        if cfg.data.modality == "audio":
                            existing_entry['audio'] = transcript
                        elif cfg.data.modality == "video":
                            existing_entry['video'] = transcript
                        existing_entry['gt'] = gt_transcript
                    else:
                        # Create a new entry
                        new_entry = {'file_path': file_path, 'audio': '', 'video': '', 'gt': gt_transcript}
                        if cfg.data.modality == "audio":
                            new_entry['audio'] = transcript
                        elif cfg.data.modality == "video":
                    ...
                    if cfg.data.modality == "audio":
                        print(f"File: {file_path}, Audio Transcript: {transcript}")
                    elif cfg.data.modality == "video":
